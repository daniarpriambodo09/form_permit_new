...
//   - divalidasi belum ada Hot Work lain yang terhubung ke id_ijin_kerja
//     yang sama (maksimal 1 form per jenis kerja per Ijin Kerja Eksternal)
//   - id_ijin_kerja disimpan ke kolom baru form_kerja_panas.id_ijin_kerja
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (idIjinKerja) {
      tipePerusahaan = 'eksternal'; // form yang terhubung ke ijin kerja eksternal selalu eksternal
      ...
      const existing = await queryOne(
        `SELECT id_form FROM form_kerja_panas WHERE id_ijin_kerja = $1`,
        [idIjinKerja]
      );
      if (existing) {
        return NextResponse.json(
          { error: `Ijin Kerja Panas untuk ${idIjinKerja} sudah pernah dibuat (${existing.id_form}). Maksimal 1 form per jenis kerja.` },
          { status: 409 }
        );
      }
    }
