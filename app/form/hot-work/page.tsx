...
const YesNoRow = ({ label, fieldKey, pencegahan, setPencegahan }: {
  ...
  pencegahan: any; setPencegahan: (p: any) => void;
}) => (
...
const defaultForm = (): FormData => ({
  noRegistrasi: "", namaKontraktor: "", namaPekerjaNIK: "",
        ...
        >
          <div className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={formData.noRegistrasi} onChange={e => setFormData(p => ({ ...p, noRegistrasi: e.target.value }))}
                  className={inputCls} placeholder="HWP-2024-001" />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" value={formData.namaPekerjaNIK} onChange={e => setFormData(p => ({ ...p, namaPekerjaNIK: e.target.value }))}
        ...
        >
          <div className="space-y-6">
            ...
            <div className="border border-slate-200 rounded-lg p-4">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <YesNoRow key={item.key} label={item.label} fieldKey={item.key} pencegahan={formData.pencegahan} setPencegahan={setP} />
                ))}
              </div>
            </div>
            ...
            <div className="border border-slate-200 rounded-lg p-4">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">Kondisi Fire Blanket / Perisai Metal</label>
                  <div className="flex gap-6">
                    {[{ v: "layak", l: "Layak" }, { v: "tidak_layak", l: "Tidak Layak" }].map(({ v, l }) => (
                      <label key={v} className="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="fireblank" value={v}
                          checked={formData.pencegahan.fireblank === v}
                          onChange={() => setP({ fireblank: v })}
                          className="w-4 h-4 text-orange-600"
                        />
                ...
                <div>
                  ...
                  <input type="number" value={formData.pencegahan.fireblank_jumlah}
                    onChange={e => setP({ fireblank_jumlah: e.target.value })}
                    className={inputCls} placeholder="Jumlah" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">Permintaan Tambahan Tindakan Pengamanan</label>
                  <textarea rows={2} value={formData.pencegahan.permintaan_tambahan}
