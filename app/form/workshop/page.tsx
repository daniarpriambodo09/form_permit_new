...
export default function WorkshopPermitForm() {
  ...
  const handleSave = async () => {
    try {
      ...
      alert(`Draft disimpan! ID: ${r.id_form}`);
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      <div className="bg-white shadow-sm border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/my-forms" className="p-2 hover:bg-slate-100 rounded-lg">
          ...
          <Link href="/form-permit/my-forms" className="px-4 py-2 text-sm font-medium text-orange-600 hover:bg-orange-50 rounded-lg">
            Lihat Riwayat →
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex justify-between items-center gap-4 bg-white p-6 rounded-xl shadow-md sticky bottom-4 border border-slate-200">
          <button onClick={handleSave} disabled={submitting || jsaUploadStatus === "uploading"} className="px-6 py-3 border-2 border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50 font-semibold flex items-center gap-2 disabled:opacity-60">
            <Save className="w-5 h-5" />{submitting ? "Menyimpan..." : "Simpan Draft"}
          </button>
          <button onClick={handleSubmit} disabled={submitting || jsaUploadStatus === "uploading"} className="px-8 py-3 bg-gradient-to-r from-orange-600 to-orange-700 hover:from-orange-700 hover:to-orange-800 text-white rounded-lg font-semibold flex items-center gap-2 shadow-lg disabled:opacity-60">
