// app/login/approver/LoginPage.tsx  (atau app/login/LoginPage.tsx)
"use client";
...
export default function LoginPage() {
  ...
  // FIX: Default 'from' harus path relatif (tanpa basePath).
  // Next.js router.replace() otomatis prepend basePath.
  // Validasi untuk mencegah open redirect.
  const rawFrom = searchParams.get("from") || "";
  const from    = rawFrom.startsWith("/form-permit/")
    ? rawFrom.replace("/form-permit", "") || "/approval"
    : rawFrom.startsWith("/")
      ? rawFrom
      : "/approval";
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/form-permit/api/auth/login", {
        method:  "POST",
        // FIX: credentials: "include" wajib agar cookie tersimpan di browser
        credentials: "include",
      ...
      // FIX: Gunakan router.replace() untuk menghindari back-button loop
      router.replace(from);
