...
export default function WorkerLoginPage() {
  ...
  const from         = searchParams.get("from") || "/my-forms";
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user.role !== "worker") {
        // ❌ CLEAR TOKEN jika role bukan worker
        await fetch("/api/auth/logout", { method: "POST" });
        sessionStorage.clear();
        setError("Akun ini bukan pekerja. Silakan gunakan halaman login approver.");
        return;
      }
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 shadow-2xl">
          <div className="mb-6">
            ...
            <p className="text-slate-400 text-xs mt-2">
              Masuk dengan akun pekerja Anda untuk mengajukan dan melacak form izin kerja.
            </p>
          </div>
